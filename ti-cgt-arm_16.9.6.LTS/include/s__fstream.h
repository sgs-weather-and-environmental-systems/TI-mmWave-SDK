...
protected:                      // Static data members.
  static _DATA_ACCESS size_t _M_page_size;
...
#if !defined (_STLP_NO_EXTENSIONS) && (!defined(__TI_COMPILER_VERSION__) || defined(__BUILDING_STLPORT))
  /**************************************************************************/
  /* For Texas Instruments, disable access to the file descriptor from the  */
  /* user level API since opening is not supported except for files already */
  /* opened with fopen.                                                     */
  /**************************************************************************/
...
private:                        // Data members used only in putback mode.
  ...
  enum __attribute__((__packed__)) { _S_pback_buf_size = 8 };
