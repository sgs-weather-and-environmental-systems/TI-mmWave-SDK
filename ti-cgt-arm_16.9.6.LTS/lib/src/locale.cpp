...
#ifndef _STATIC_LOCALE_DATA_DEFINED
/*****************************************************************************/
/* This data is defined in both locale.cx and locale_impl.cx.  As separate   */
/* compilation units, it may be redefined, but must only be defined once     */
/* when compiled together with unified_locale.cpp.                           */
/*****************************************************************************/
static const string _Nameless("*");

static inline bool is_C_locale_name (const char* name)
{ return ((name[0] == 'C') && (name[1] == 0)); }
#define _STATIC_LOCALE_DATA_DEFINED
#endif
