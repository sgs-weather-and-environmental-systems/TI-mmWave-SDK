...
log1pf(float x)
{
	...
	if (hx < 0x3ed413d0) {			/* 1+x < sqrt(2)+  */
		...
		if(x==(float)-1.0) return -INFINITY; /* log1p(-1)=-inf */
		else
                {
                    __raise_invalid();
...
double log1p(double x) __attribute__((__alias__("log1pf")));
#endif

#if LDBL_MANT_DIG == FLT_MANT_DIG
long double log1pl(long double x) __attribute__((__alias__("log1pf")));
#endif
