...
@echo For Example:
@echo   set MMWAVE_SDK_INSTALL_PATH=C:/ti/mmwave_sdk_01_02_00_05/packages
goto fail
)
...
@echo For Example:
@echo   set BIOS_INSTALL_PATH=C:/ti/bios_6_53_02_00/packages
goto fail
)
if [%XDC_INSTALL_PATH%] == [] (
@echo Error: XDC_INSTALL_PATH [XDC Build Tools] needs to be defined
@echo For Example:
@echo   set XDC_INSTALL_PATH=C:/ti/xdctools_3_50_04_43_core
goto fail
)
...
@echo Error: MMWAVE_SDK_DEVICE needs to be defined to awr14xx, awr16xx, iwr14xx or iwr16xx
@echo For Example:
...
if [%MMWAVE_SDK_DEVICE_TYPE%] == [xwr14xx] (
  if NOT [%MMWAVE_L3RAM_SIZE%] == [] (
  set L3RAM_SIZE=%MMWAVE_L3RAM_SIZE%
  )
)
...
set PATH=%XDC_INSTALL_PATH%;%XDC_INSTALL_PATH%/bin;%PATH%

REM Tools:
REM - Doxygen
REM These are only used by the release scripts. Customers can build without these defined.
set PATH=%DOXYGEN_INSTALL_PATH%\bin;%GRAPHVIZ_INSTALL_PATH%\bin;%MIKTEX_INSTALL_PATH%;%GHOSTSCRIPT_INSTALL_PATH%\bin;%PATH%
