...
extern void test_dftsinglebin();
extern void test_dftsinglebinwithwin();
...
void benchmark_64pt_2dprocess(uint16_t refRangeBin)
{
	...
	for (i = refRangeBin-2; i<=refRangeBin; i++)
	{
		...
		printf("%d-point Win16x32 cycles =%d; FFT32x32 cycles =%d; logabs cycles =%d; accum16 cycles =%d\n", numDopplerBins, wincyc, fftcyc, logcyc, accumcyc);
	}
}
...
void test_window2d_dopplerfft_log2abs(int32_t numRangeBins, int32_t numDopplerBins, uint32_t cfardBc1 )
{
	...
	if ( testFailed ==0 )
	{
		printf("\nTest 2D windowing16x32, FFT32x32, log2Abs32 passed.\n");
	}
	else
		totalFailed += testFailed;
