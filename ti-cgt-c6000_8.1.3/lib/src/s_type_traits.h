...
#  include <s_type_manips.h>
#endif

#ifdef _STLP_USE_BOOST_SUPPORT
#  include <s_boost_type_traits.h>
...
struct _IsPtr {
  ...
  enum __attribute__((__packed__)) { _Ptr = (sizeof(_IsP(false,__null_rep())) == sizeof(char)) };
...
template <class _Tp>
struct _IsStateless {
  enum __attribute__((__packed__)) { _Is = sizeof(_TpWithState<_Tp>) == sizeof(int) };
