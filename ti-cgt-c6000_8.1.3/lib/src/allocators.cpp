...
#if defined (__TI_COMPILER_VERSION__) && !defined(__TI_EABI__)
/******************************************************************************/
/* Workaround for error 1369: "static local variables of extern inline        */
/* function are not resolved to single copy. Not ANSI C++ Compliant"          */
/******************************************************************************/
static _STLP_STATIC_MUTEX _Node_Alloc_Lock_mutex__;
static int _Node_Alloc_Lock_mutex_inited__ = 0;

class _Node_Alloc_Lock {
  static _STLP_STATIC_MUTEX& _S_Mutex() {
    if (_Node_Alloc_Lock_mutex_inited__ == 0)
    {
        #pragma diag_suppress 176 /* Expression has no effect */
        _Node_Alloc_Lock_mutex__ _STLP_MUTEX_INITIALIZER;
        _Node_Alloc_Lock_mutex_inited__ = 1;
    }
    return _Node_Alloc_Lock_mutex__;
  }
#else
class _Node_Alloc_Lock {
  static _STLP_STATIC_MUTEX& _S_Mutex() {
